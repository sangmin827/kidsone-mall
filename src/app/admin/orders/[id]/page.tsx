...
import { getAdminOrderById, updateOrderStatus, updateOrderMemo, updateOrderTracking } from "@/src/server/orders";

// ── 택배사 목록 ──────────────────────────────────────────────────────────
const COURIERS: { code: string; name: string; url: (n: string) => string }[] = [
  { code: "cj",      name: "CJ대한통운",    url: (n) => `https://trace.cjlogistics.com/next/tracking.html?wblNum=${n}` },
  { code: "hanjin",  name: "한진택배",       url: (n) => `https://www.hanjin.com/kor/CMS/DeliveryMgr/WaybillResult.do?mCode=MN038&schLang=KOR&wblnumText=${n}` },
  { code: "lotte",   name: "롯데택배",       url: (n) => `https://www.lotteglogis.com/mobile/reservation/tracking/linkView?InvNo=${n}` },
  ...
  { code: "kdexp",   name: "경동택배",       url: (n) => `https://kdexp.com/m/service/delivery/delivery.do?barcode=${n}` },
  { code: "cu",      name: "CU편의점택배",   url: (n) => `https://www.cupost.co.kr/postbox/delivery/localResult.cupost?transNo=${n}` },
  { code: "gs",      name: "GS25편의점택배", url: (n) => `https://www.cvsnet.co.kr/invoice/tracking.do?invoice_no=${n}` },
  { code: "hyundai", name: "현대택배",       url: (n) => `https://www.hdexp.co.kr/parcel/goods_search/delivery_search.do?slipNoSearch=${n}` },
];
            ...
            {order.tracking_number && (
              <div className="mb-3 rounded-xl border border-blue-100 bg-blue-50 p-3">
                <p className="text-[11px] text-blue-500 font-medium mb-1">
                  {COURIERS.find((c) => c.code === order.courier_code)?.name ?? order.courier_code}
                </p>
                <p className="font-mono text-sm font-bold text-[#222222]">{order.tracking_number}</p>
